// ============================================
// AI Panel Studio — Mock Panelist Generator
// ============================================
// Deterministic mock panelist generation.
...
/**
 * Generate panelists deterministically from preset personas.
 * Returns 1 moderator + N experts based on topic hash.
 */
export function generateMockPanelists(
  topic: string,
  expertCount: number
): GeneratedPanelist[] {
  const seed = topicHash(topic)
  ...
  const chosen = shuffled.slice(0, expertCount)
  for (let i = 0; i < chosen.length; i++) {
    ...
    panelists.push({
      ...
      name: expert.name,
      title: expert.title,
      stance: fillStance(expert.stance_templates[stanceIdx], topic),
      color: expert.color,
    })
  }
